...
class MessagesController extends AppController {
	...
	public function index() {
		$this->paginate = array(
        	...
        	'conditions' => array('Message.created = Maxdate.maxdate'),
	...
	public function delcon($id = null) {
		if ($this->request->is('ajax')) {
			...
			if ($this->Message->deleteAll(array('Message.con_id' => $id))) {
				$data['status'] = true;
    ...
    public function from($id) {
        ...
        if (!$fromPerson) {
        	...
        	if ($this->request->is(array('post', 'put'))) {
	        	...
	        	$this->set(array('messages' => $this->Paginator->paginate(), 'fromPerson' => $fromPerson));
	        	if ($this->request->is('ajax')) {
